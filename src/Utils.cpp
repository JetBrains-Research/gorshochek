...
std::unique_ptr<FrontendActionFactory> newBaseFrontendActionFactory(
        vector<BaseTransformation> transformations, string output_path) {
    class SimpleFrontendActionFactory : public FrontendActionFactory {
    public:
        SimpleFrontendActionFactory(vector<BaseTransformation> transformations, \
                                    string output_path) \
                                    : transformations(transformations), output_path(output_path) {}
    ...
    return std::unique_ptr<FrontendActionFactory>(
            new SimpleFrontendActionFactory(transformations, output_path));
}

Runner::Runner(int argc, const char **argv) : argc(argc), argv(argv) {};

vector<BaseTransformation> Runner::getTransformations(const string& config_path){
